...
const ArtistPage = () => {
  ...
  const displayName = data.heading.replace(/^Artists Like\s*/i, "");
  ...
  return (
    <div className="min-h-screen px-4 py-12 max-w-3xl mx-auto space-y-10">
      ...
      {data.closest_matches.length > 0 && (
        <ResultSection title="Closest Matches" items={data.closest_matches} linkPrefix="/artists-like" />
      )}
      {data.same_energy.length > 0 && (
        <ResultSection title="Same Energy" items={data.same_energy} linkPrefix="/artists-like" />
      )}
